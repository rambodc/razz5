// Firebase Cloud Function: v1_update_email_verified.js
...
async function initialize_and_check(req, res) {
    ...
    try {
        ...
        return { proceed: true, uid, email_verified };
    } catch (error) {
...
module.exports.v1_update_email_verified = functions.https.onRequest(async (req, res) => {
    ...
    const { uid, email_verified } = init_result;
    const user_ref = admin.firestore().collection('users').doc(uid);

    try {
        console.log(`Starting v1_update_email_verified function for uid: ${uid}`);
        ...
        await admin.firestore().runTransaction(async (transaction) => {
            ...
            // Update the email_verified field in the user's document
        ...
        return res.status(200).send({ status: "success", message: "User email verification status updated successfully", email_verified });
    } catch (error) {
        console.error(`Error in v1_update_email_verified function for uid: ${uid}: ${error.message}`);
