...
module.exports = async function lastResetTime(documentId, params) {
    const { lastResetTime, dailyRaz } = params;
    ...
    try {
        ...
        await admin.firestore().runTransaction(async (transaction) => {
            ...
            transaction.update(userRef, { lastResetTime, dailyRaz });

            // Update the functionCalls document with the completion status
            transaction.update(functionCallRef, {
                'status': 'completed',
                'response.result': 'success',
                'response.data.lastResetTime': lastResetTime,
                'response.data.dailyRaz': dailyRaz,
                'response.completed': admin.firestore.FieldValue.serverTimestamp()
            });
        });
    } catch (error) {
        console.error(`Error in lastResetTime function: ${error.message}`);
